...
def make_chart_image(df_matched):
    """Return base64 PNG of the course chart using matplotlib (no kaleido needed)."""
    try:
        ...
        colors = ["#2563EB" if i == 0 else "#60A5FA" for i in range(n)]
        bars = ax.barh(course_stats["CourseName"], course_stats["Revenue"],
                       color=colors, edgecolor="none", height=0.55)

        for bar, students in zip(bars, course_stats["Students"]):
            ax.text(bar.get_width() + max(course_stats["Revenue"]) * 0.01,
                    bar.get_y() + bar.get_height()/2,
                    f"👤 {int(students)} student{'s' if students!=1 else ''}",
                    va="center", ha="left", fontsize=9, color="#374151")

        ax.set_xlabel("Revenue (₹)", fontsize=10, color="#374151")
        ax.set_title("Course Sales Performance", fontsize=13, fontweight="bold", color="#0F172A", pad=12)
        ...
        ax.set_xlim(0, max(course_stats["Revenue"]) * 1.25)
        plt.tight_layout()
